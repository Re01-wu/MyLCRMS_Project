...
public class SysUserController extends BaseController {
    ...
    public AjaxResult add(@Validated @RequestBody SysUser user) {
        // 在 ruoyi-admin 模块中，可能不需要检查部门和角色数据范围，或者检查方式不同
    ...
    public AjaxResult listAllSimple() {
        // 查询未被删除的用户 (可以根据需要添加其他过滤条件，比如特定角色)
        SysUser userQuery = new SysUser();
        userQuery.setDelFlag("0"); // 只查询未删除的用户
        List<SysUser> list = userService.selectUserList(userQuery); // 调用注入的 userService 获取列表

        // 转换为 { value: userId, label: nickName } 格式
        List<Map<String, Object>> options = list.stream().map(user -> {
            Map<String, Object> map = new HashMap<>();
            map.put("value", user.getUserId());
            map.put("label", user.getNickName()); // 使用昵称作为显示标签
            return map;
